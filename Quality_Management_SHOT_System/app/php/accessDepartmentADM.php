...
/* This will automatically open a database connection and check if the session has expired */
  ...
  elseif ($Action == "U")
  {
    ...
    $stmtx = $mysqli->prepare("UPDATE officer_department set Department = ? WHERE Department_ID = ?");
    if ( false===$stmtx ) {
      trigger_error($emsg);
    }

    $rc = $stmtx->bind_param('si', $Department, $Department_ID);
    if (false===$rc)
    {
      trigger_error($emsg);
    }
  ...
  elseif ($Action == "D")
  {
    ...
    $stmtx = $mysqli->prepare("DELETE FROM officer_department WHERE Department_ID = ?");
    if ( false===$stmtx ) {
      trigger_error($emsg);
    }

    $rc = $stmtx->bind_param('i', $Department_ID);
    if (false===$rc)
    {
      trigger_error($emsg);
    }
